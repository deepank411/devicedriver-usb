...
static int pen_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
    ...
    if ((retval = usb_register_dev(interface, &class)) < 0)
    {
        ...
        printk(KERN_ERR "Not able to register this device.");
    }
    else
    {
...
static int __init pen_init(void)
{
    ...
    if ((result = usb_register(&pen_driver))) //assignment not comparison
    {
...
MODULE_DESCRIPTION("USB Pen Drive Device Driver");
